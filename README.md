...
- 사용자가 본인 클로젯 제품과 사진을 등록하면 학습된 YOLO 탐지 모델과 OpenAI 멀티모달 비전 모델이 각각 분석합니다.
- 두 모델의 손상 종류·개수·위치가 일치하면 해당 결과를 사용합니다. 일치하지 않으면 두 모델을 한 번 더 실행하고, 두 번째 결과도 다르면 AI 결과를 사용합니다.
- 진단 상태는 생성 시 `PENDING`이며 별도 워커의 분석 성공 시 `DONE`, 실패 시 `FAILED`로 자동 변경됩니다. 프론트엔드는 상세 조회를 반복해 완료 상태를 확인합니다.
- 결과는 안전·주의·위험 3단계와 손상 유형·설명·관리 제안으로 제공합니다.
- 사진 위 표시를 위해 탐지 영역과 가장 중요한 손상 위치를 백분율 좌표로 반환합니다.
