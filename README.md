...
| POST | `/api/auth/token/refresh/` | JWT 액세스 토큰 갱신 |
| GET/PATCH | `/api/me/` | 프로필·온보딩·멤버십 등급 조회 및 수정 |
| GET/PATCH | `/api/notifications/` | 알림 목록·상세 조회 및 읽음 처리 |
| CRUD | `/api/products/` | 내 클로젯 제품 등록·조회·수정·삭제 |
| CRUD | `/api/product-images/` | 제품·영수증·보증서 이미지 관리 |
| GET | `/api/care-guides/` | 기본·구매 직후·사후 케어 가이드 조회 |
| GET/POST/DELETE | `/api/care-bookmarks/` | 케어 가이드 북마크 조회·등록·삭제 |
| CRUD | `/api/diagnoses/` | AI 케어 진단 요청·이력 조회·수정·삭제 |
| GET | `/api/stores/` | AS 지원 공식 케어 매장 목록·상세 조회 |
| CRUD | `/api/visit-reservations/` | AS 방문 예약 생성·조회·수정·삭제 |
| GET | `/api/visit-reservations/availability/` | 매장과 날짜별 예약 가능 시간 조회 |
| POST | `/api/visit-reservations/{id}/cancel/` | AS 방문 예약 취소 |
| CRUD | `/api/service-requests/` | 내부 AS 서비스 요청 관리 |
| CRUD | `/api/posts/` | 커뮤니티 게시글 및 상품 태그 관리 |
| CRUD | `/api/post-images/` | 커뮤니티 게시글 다중 이미지 관리 |
| CRUD | `/api/comments/` | 커뮤니티 댓글 관리 |
| GET/POST/DELETE | `/api/post-likes/` | 게시글 좋아요 조회·등록·취소 |
| POST | `/api/ai/chat/` | AI 챗봇 메시지 요청 |
| CRUD | `/api/ai/chat-sessions/` | AI 챗봇 대화 세션 관리 |
