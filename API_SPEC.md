...
응답의 `consultation_summary`는 서버가 상담에서 사용자가 말한 색상·사이즈·착용 상황·예산 등의 니즈와 선택한 상품 정보를 OpenAI에 전달해 2~4문장의 개인화된 방문 카드 문구로 생성합니다. AI 추천 답변 원문, 여러 상품 목록, Markdown 및 이미지 URL은 저장하지 않습니다. 클라이언트가 보낸 `consultation_summary` 값은 저장에 사용하지 않습니다. 요약 생성에 실패해도 방문 카드는 정상적으로 저장되며 `consultation_summary`에는 `니즈를 파악중입니다..`가 저장됩니다.
